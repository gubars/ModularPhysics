...
This file develops the theory of vector bundles over the moduli space of
Riemann surfaces M_{g,n}, which is essential for:
- Donagi-Witten theorem (Hodge bundle and Sym²E)
- String amplitudes (ψ-classes and cotangent line bundles)
- Intersection theory on M̄_{g,n}
...
* `PsiClass` - The first Chern class ψᵢ = c₁(Lᵢ)
...
At each marked point pᵢ, the cotangent line bundle Lᵢ has fiber T*_{pᵢ}Σ.
Its first Chern class ψᵢ = c₁(Lᵢ) is a fundamental class on M̄_{g,n}.

### The Tautological Ring

The tautological ring R*(M̄_{g,n}) is generated by:
- ψ-classes: ψᵢ = c₁(Lᵢ)
- κ-classes: κₐ = π_*(ψ^{a+1}) where π : M_{g,n+1} → M_{g,n}
- λ-classes: λₖ = cₖ(E) (Chern classes of Hodge bundle)

Intersection numbers ⟨τ_{d₁}...τ_{dₙ}⟩_g = ∫_{M̄_{g,n}} ψ₁^{d₁}...ψₙ^{dₙ}
are computed by the Witten-Kontsevich theorem.

## References

* Harris, Morrison - "Moduli of Curves"
* Arbarello, Cornalba, Griffiths - "Geometry of Algebraic Curves II"
* Witten - "Two-dimensional gravity and intersection theory"
-/
